...
class SupplierListNewOrderLine extends Component {
  constructor(props) {
    super(props);
    this.state = {
      token: '',
  ...
  addToBasketFunSec = () => {
    const {finalBasketData, supplierId, basketId, productId, supplierName} =
      this.state;

    console.log('finalBasketData', finalBasketData);

    if (finalBasketData.length > 0) {
      addNewOrderLineApi(finalBasketData)
        .then(res => {
          this.setState(
            {
              loaderCompStatus: false,
